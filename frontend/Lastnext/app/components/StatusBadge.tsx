...
type StatusTone = 'green' | 'blue' | 'amber' | 'orange' | 'purple' | 'red' | 'gray';
...
const statusConfig: Record<string, StatusBadgeConfig> = {
  ...
  waiting_fix_defect: { label: 'Waiting Fix Defect', tone: 'purple' },
...
export function getStatusBadgeConfig(status?: string): StatusBadgeConfig {
  ...
  return statusConfig[normalized] || { label: humanize(normalized), tone: 'gray' };
}
...
export function StatusBadge({ status, className, ...props }: StatusBadgeProps) {
  ...
  return (
    <span
      className={cn(
        'inline-flex min-h-8 min-w-0 max-w-full items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold leading-snug shadow-sm sm:min-h-9 sm:px-3 sm:text-sm',
      ...
      {...props}
    >
      <span className={cn('h-2 w-2 flex-none rounded-full', dotClasses[config.tone])} aria-hidden="true" />
