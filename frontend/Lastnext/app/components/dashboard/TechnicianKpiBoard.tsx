...
export function TechnicianKpiBoard({ jobs, windowDays = 7, className }: TechnicianKpiBoardProps) {
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    for (const job of jobs) {
      ...
      const overdue = !job.completed_at && !Number.isNaN(created)
        && (now - created) / dayMs >= 3
    ...
    return Array.from(buckets.values()).map((row) => {
      ...
      const avg = row.responseSamples.length
        ? row.responseSamples.reduce((sum, value) => sum + value, 0) / row.responseSamples.length
        : null;
      return {
        key: row.key,
        name: row.name,
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs font-medium text-slate-500">
              {rows.length} technician{rows.length === 1 ? '' : 's'} · last {windowDays} days vs prior {windowDays}
            </p>
          </div>
        </div>
        {top && (
      ...
      <ul className="space-y-2 md:hidden">
        {sorted.map((row) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="text-[11px] font-semibold text-slate-500">
                  {row.assigned} assigned · {row.completed} done · {row.overdue} overdue
                </p>
              </div>
              <span
                className={cn(
            ...
            <div className="mt-2 grid grid-cols-3 gap-1 text-[11px] font-semibold text-slate-600">
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">
            <tr>
              ...
              <HeaderCell label="Assigned" columnKey="assigned" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />
              <HeaderCell label="Done" columnKey="completed" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />
              <th className="px-3 py-2">Overdue</th>
              <HeaderCell label="Rate" columnKey="rate" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />
              <HeaderCell label="Avg response" columnKey="avgResponseHours" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />
...
function HeaderCell({ label, columnKey, sortKey, sortDir, onSort }: HeaderCellProps) {
  ...
  return (
    <th className="px-3 py-2">
      ...
      >
        ...
        {active && (
          <span className="text-[9px] font-bold">{sortDir === 'asc' ? '↑' : '↓'}</span>
        )}
      </button>
    </th>
  );
