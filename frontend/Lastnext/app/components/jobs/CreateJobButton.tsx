...
const validationSchema = Yup.object().shape({
  ...
  room: Yup.object()
    .shape({
      room_id: Yup.number()
        .required("Room must be selected")
        .min(1, "Room must be selected"),
      // Add other required fields from Room if necessary for validation context
      name: Yup.string().optional(),
      room_type: Yup.string().optional(),
    })
    .defined()
    .required("Room selection is required"), // Ensure the object itself is required
  files: Yup.array()
...
const initialValues: FormValues = {
  ...
  room: {
    // Ensure initial room aligns with Room type and validation needs
...
const CreateJobButton: React.FC<CreateJobButtonProps> = ({
  propertyId,
  onJobCreated,
}) => {
  ...
  const fetchData = async (currentPropertyId: string) => {
    ...
    try {
      ...
      const [roomsResponse, topicsResponse] = await Promise.all([
        ...
        apiClient.get<TopicFromAPI[]>(
          `/api/topics/?property=${encodeURIComponent(currentPropertyId)}`,
        ),
      ]);
      ...
      setError(
        "Failed to fetch necessary data (Rooms/Topics). Please try again.",
      );
      // Clear data on error
      setRooms([]);
      setTopics([]);
    ...
    try {
      ...
      const payload = {
        ...
        topic_data: JSON.stringify({
          // Send topic details as JSON string
  ...
  const handleAuthClick = async () => {
    if (status === "unauthenticated") {
      await signIn(); // Redirects to /auth/login via compat
      return;
    }
  ...
  return (
    ...
    >
      <DialogTrigger asChild>
        <Button
          className="h-12 px-6 flex items-center gap-2 rounded-[14px] shadow-card sm:w-auto w-full"
        ...
        {status === "authenticated" ? (
          ...
          >
            ...
            {({
              ...
              <Form className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto pr-2">
                {" "}
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="waiting_sparepart">
                          Waiting Sparepart
                        </SelectItem>
                ...
                <div className="items-top flex space-x-2 mt-2">
                  ...
                  <div className="grid gap-1.5 leading-none">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Check this if the item needs external repair or is part of
                      a defect list.
                    </p>
                  </div>
                </div>
                <div className="mt-2">
                  ...
                  <div
                    className={`transition-all duration-300 ease-in-out ${showRemarks ? "max-h-40 opacity-100" : "max-h-0 opacity-0"} overflow-hidden`}
                  >
                    <Field
                      as={Textarea}
                      id="remarks"
                ...
                <div className="flex justify-end gap-4 pt-4 border-t mt-4">
                  ...
                  <Button
                    ...
                    isLoading={formikIsSubmitting || isSubmitting}
