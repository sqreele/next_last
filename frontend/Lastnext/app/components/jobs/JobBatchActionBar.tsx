...
import { CheckCircle2, FileSpreadsheet, Loader2, X, Wrench, RefreshCw } from 'lucide-react';
...
export function JobBatchActionBar({
  ...
  const [working, setWorking] = useState<null | 'status' | 'csv' | 'xlsx'>(null);
  ...
  const applyStatus = async (newStatus: JobStatus) => {
    ...
    try {
      ...
      if (failed) {
        setError(`Updated ${selected.length - failed} of ${selected.length} jobs. ${failed} failed.`);
      } else {
        onClear();
        onComplete?.();
      }
  ...
  const exportCsv = () => {
    ...
    downloadCSV(csv, `selected-jobs-${new Date().toISOString().split('T')[0]}.csv`);
  };
  ...
  return (
    <div
      ...
      className={cn(
        'sticky bottom-3 z-40 mx-auto flex w-full max-w-3xl flex-col gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-3 shadow-2xl sm:flex-row sm:items-center sm:gap-3',
        className,
      )}
    ...
    >
      <div className="flex flex-1 items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          >
            {selected.length === totalVisible ? 'Clear selection' : 'Select all visible'}
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-end gap-2 sm:flex-none">
        <div className="relative">
          ...
          {statusPickerOpen && (
            <div className="absolute bottom-full right-0 z-50 mb-2 flex w-56 flex-col gap-1 rounded-xl border border-slate-200 bg-white p-2 shadow-xl">
              {QUICK_STATUSES.map((s) => (
                <button
                  key={s.value}
                  type="button"
                  onClick={() => applyStatus(s.value)}
                  className="flex items-center justify-between gap-2 rounded-lg px-2 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-100"
                >
                  <StatusBadge status={s.value} size="sm" />
                  <span className="text-[11px] text-slate-500">→ {selected.length} job{selected.length > 1 ? 's' : ''}</span>
                </button>
              ))}
            </div>
          )}
