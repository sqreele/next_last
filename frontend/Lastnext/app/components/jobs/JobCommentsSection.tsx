...
const JobCommentsSection: React.FC<Props> = ({ jobId }) => {
  ...
  const pendingComments = queue.filter(
    (item) =>
      ...
      item.endpoint === `/api/v1/jobs/${jobId}/comments/`,
  );
  ...
  const fetchComments = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await axios.get(`/api/jobs/${jobId}/comments/`, {
        withCredentials: true,
      });
      const data = res.data;
      const list: JobComment[] = Array.isArray(data)
        ? data
        : data?.results || [];
      setComments(list);
    } catch (err) {
  ...
  return (
    <section
      className="pcms-section-card space-y-4 p-5 sm:p-6"
        ...
        <ol className="space-y-3">
          ...
          {pendingComments
            ...
            .map((item) => (
              <li
                key={item.id}
                className="rounded-lg border border-amber-200 bg-amber-50 p-3 shadow-soft"
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="text-sm font-semibold text-foreground">
                    You
