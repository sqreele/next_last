...
const TopicAutocomplete: React.FC<TopicAutocompleteProps> = ({
  topics,
  selectedTopic,
  onSelect,
}) => {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && filteredTopics.length > 0 && (
        <div className="absolute z-10 w-full max-h-60 overflow-y-auto bg-card border border-border shadow-soft rounded-md mt-1">
          {filteredTopics.map((topic) => (
            <div
              key={topic.id}
              className="px-4 py-2.5 text-base hover:bg-muted cursor-pointer"
              onClick={() => {
                onSelect({
                  title: topic.title,
      ...
      {isOpen && filteredTopics.length === 0 && (
        <div className="absolute z-10 w-full bg-card border border-border shadow-soft rounded-md mt-1 p-2 text-sm text-muted-foreground">
          No topics found
        </div>
      )}
