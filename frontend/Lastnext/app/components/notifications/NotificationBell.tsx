  ...
  return (
    <div ref={containerRef} className={cn("relative inline-flex", className)}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -right-0.5 -top-0.5 grid h-5 min-w-[1.25rem] place-items-center rounded-full bg-rose-600 px-1 text-[10px] font-bold text-white shadow-sm ring-2 ring-white">
      ...
      {open && (
        <div
          ...
          className="absolute right-0 top-full z-50 mt-2 w-[min(22rem,calc(100vw-1.5rem))] overflow-hidden rounded-xl border border-border bg-card shadow-card"
        >
          <header className="flex items-center justify-between border-b border-border bg-muted px-4 py-3">
            ...
            <button
              ...
              className="grid h-7 w-7 place-items-center rounded-full text-muted-foreground hover:bg-slate-200 hover:text-foreground"
          ...
          <div className="max-h-[60vh] overflow-y-auto">
            {loading && !data ? (
              ...
              <ul className="divide-y divide-slate-100">
                {(
                  ["overdue", "today", "upcoming", "completed"] as const
                ).flatMap((kind) =>
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs font-semibold text-muted-foreground">
                              #{item.pm_id} · {styles.label} ·{" "}
                              {relativeTime(item.scheduled_date)}
                            </p>
                          </div>
                        </Link>
                      </li>
          ...
          <footer className="flex items-center justify-between border-t border-border bg-muted px-4 py-2 text-xs font-bold">
            <button
              type="button"
              onClick={() => {
                setReadSet(() => {
                  const next = new Set<string>(
                    (data?.results || []).map((n) => n.pm_id),
                  );
                  saveReadSet(next);
                  return next;
                });
