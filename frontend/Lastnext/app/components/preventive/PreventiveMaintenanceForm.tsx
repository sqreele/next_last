...
const PreventiveMaintenanceForm: React.FC<PreventiveMaintenanceFormProps> = ({
  ...
  const calculateNextScheduledDate = useCallback(
    (frequency: string, customDays?: number, baseDate?: Date): Date => {
  ...
  const convertToISO8601 = useCallback((dateTimeLocal: string): string => {
    ...
    try {
      ...
      console.error(
        "[convertToISO8601] Error converting datetime-local to ISO 8601:",
        error,
        "Input:",
        dateTimeLocal,
      );
      ...
      if (
        dateTimeLocal.includes("T") &&
        (dateTimeLocal.includes("Z") || dateTimeLocal.includes("+"))
      ) {
      ...
      console.warn(
        "[convertToISO8601] Using last resort fallback format:",
        fallback,
      );
      return fallback;
    }
  ...
  const getPropertyName = useCallback(
    (propertyId: string | null): string => {
      ...
      const foundProperty = userProperties?.find(
  ...
  const validateForm = (values: FormValues): FormikErrors<FormValues> => {
        ...
        if (isNaN(scheduledDate.getTime())) {
          ...
          if (!pmId) {
            ...
            if (scheduledDate < oneDayAgo) {
              ...
              console.warn(
                "Scheduled date is more than 1 day in the past:",
                scheduledDate,
              );
            }
          }
    ...
    if (!pmId && values.completed_date && values.completed_date.trim() !== "") {
      errors.completed_date =
        "Completed date should be empty for new maintenance records";
    }
  ...
  const getInitialValues = useCallback((): FormValues => {
    ...
    if (currentData) {
      ...
      const customDays =
        ...
        currentData.custom_days === undefined
          ? ""
          : currentData.custom_days;
  ...
  const fetchAvailableMaintenanceTasks = useCallback(
    async (machineIds?: string[]) => {
      const mergeTemplatesById = (
        templates: MaintenanceProcedureTemplate[],
  ...
  useEffect(() => {
    ...
    if (pmId && !initialDataProp) {
      ...
      preventiveMaintenanceService
        ...
        .then((response) => {
          ...
          if (response.success && response.data) {
            ...
            if (response.data.before_image_url)
              setBeforeImagePreview(response.data.before_image_url);
            if (response.data.after_image_url)
              setAfterImagePreview(response.data.after_image_url);
            if (!response.data.property_id) {
              console.warn(
                "[PreventiveMaintenanceForm] Missing property_id in maintenance data",
              );
              setError(
            ...
            throw new Error(
              response.message || "Failed to fetch maintenance data",
            );
          }
        })
        .catch((err) => {
      ...
      if (initialDataProp.before_image_url)
        setBeforeImagePreview(initialDataProp.before_image_url);
      if (initialDataProp.after_image_url)
        setAfterImagePreview(initialDataProp.after_image_url);
      if (!initialDataProp.property_id) {
        console.warn(
          "[PreventiveMaintenanceForm] Missing property_id in initialDataProp",
        );
        setError(
          "Warning: No property associated with this maintenance record. Please select one.",
        );
      ...
      <div className="w-full max-w-none px-3 py-4 sm:px-6 sm:py-6 lg:mx-auto lg:max-w-7xl lg:px-8 desktop:max-w-[96rem]">
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
          ...
          <p className="mt-2 text-sm">
            Please contact your administrator to assign properties to your
            account.
          </p>
        </div>
        <div className="mt-4">
  ...
  const handleFileSelection = (
    ...
    if (!validImageTypes.includes(file.type)) {
      toast.error(
        `Please upload an image file (JPEG, PNG, or GIF) for ${type === "before" ? "Before" : "After"} image.`,
      );
      return;
    }
    if (file.size > MAX_FILE_SIZE) {
      toast.error(
        `${type === "before" ? "Before" : "After"} image must be less than 5MB`,
      );
      return;
    }
    setFieldValue(
      type === "before" ? "before_image_file" : "after_image_file",
      file,
    );
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      if (
        ...
        values.completed_date.trim() !== "" &&
        values.status !== "completed"
      ) {
        setSubmitError(
          "Completed date should be empty when creating new maintenance records unless status is completed.",
        );
        setIsLoading(false);
      ...
      const completedDateISO =
        values.completed_date && values.completed_date.trim() !== ""
      ...
      const assignedToNumber =
        assignedToValue &&
        !isNaN(Number(assignedToValue)) &&
        Number(assignedToValue) > 0
      ...
      const dataForService: CreatePreventiveMaintenanceData = {
        ...
        procedure_template:
          values.procedure_template !== ""
            ? Number(values.procedure_template)
            : undefined,
      ...
      if (
        dataForService.frequency === "custom" &&
        !dataForService.custom_days
      ) {
      ...
      try {
        if (maintenanceIdToUpdate) {
          ...
          response =
            await preventiveMaintenanceService.createPreventiveMaintenance(
              dataForService,
            );
        }
      } catch (apiError: any) {
      ...
      if (error.code === "ECONNABORTED" || error.message?.includes("timeout")) {
        ...
        if (typeof responseData === "string") {
          ...
          const fieldErrors = Object.entries(responseData)
            ...
            .map(([field, errs]) => {
              const errorText = Array.isArray(errs)
                ? errs.join(", ")
                : String(errs);
            ...
            >
              ...
              {!values.property_id && !pmId && (
                <div className="mb-4 sm:mb-6 rounded-md border border-amber-200 bg-amber-50 p-3 sm:p-4 text-xs sm:text-sm text-amber-800">
                  Please select a property using the header dropdown before
                    ...
                    if (taskId) {
                      ...
                      if (selectedTask) {
                        ...
                        if (
                          ...
                          selectedTask.frequency.trim() !== "" &&
                          selectedTask.frequency !== "N/A"
                        ) {
                          const templateFrequency = selectedTask.frequency
                            .toLowerCase()
                            .trim();
                          ...
                          if (
                            ...
                            console.warn(
                              `[Template Selection] Invalid frequency "${selectedTask.frequency}" from template, using default`,
                            );
                          }
                        } else {
                          console.warn(
                            `[Template Selection] Template has no valid frequency, keeping current values`,
                          );
                        }
                      } else {
                        console.warn(
                          `[Template Selection] Template with ID ${taskId} not found in available tasks`,
                        );
                      }
                    } else {
                ...
                {!loadingMaintenanceTasks &&
                  ...
                  values.selected_machine_ids.length > 0 && (
                    <p className="mt-1 text-xs text-amber-600">
                      No tasks match the selected machine(s). Tasks are filtered
                      by machine group_id or linked procedures.
                    </p>
                  )}
              </div>
              ...
              {pmId && (
                <div className="mb-4 sm:mb-6">
                  ...
                  <Field
                    ...
                    id="completed_date"
                    name="completed_date"
                    className="w-full p-2.5 sm:p-3 text-base sm:text-sm border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
              ...
              <div className="mb-4 sm:mb-6">
                ...
                <Field
                  ...
                  name="notes"
                  rows={4}
                  className="w-full p-2.5 sm:p-3 text-base sm:text-sm border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y"
              ...
              {values.procedure_template === "" ? (
                ...
                <div className="mb-4 sm:mb-6">
                  ...
                  <div
                    className={`border rounded-md p-3 sm:p-4 max-h-60 overflow-y-auto bg-card scroll-momentum ${
                      errors.selected_machine_ids &&
                      touched.selected_machine_ids
                        ? "border-red-500"
                        : "border-border"
                    }`}
                    role="group"
                  ...
                  >
                    {!values.property_id ? (
                      ...
                      (() => {
                        ...
                        const selectedTaskMachineIds = new Set(
                          (selectedTask?.machine_ids ?? []).map((id) =>
                            String(id),
                          ),
                        );
                        const machinesMatchingTemplate =
                          selectedTaskMachineIds.size > 0
                            ? availableMachines.filter((m) =>
                                selectedTaskMachineIds.has(
                                  String(m.machine_id),
                                ),
                              )
                            : selectedTaskGroupId
                        ...
                        return machinesToShow.length > 0 ? (
                          <div className="space-y-2 sm:space-y-3">
                            {machinesToShow.map((machineItem) => {
                              const isPreSelected =
                                !!machineId &&
                                machineItem.machine_id === machineId;
                              return (
                                <div
                                ...
                                >
                                  ...
                                  >
                                    <Field name="selected_machine_ids">
                                      {({
                                        field: { value: selectedMachinesValue },
                                        form: {
                                          setFieldValue: setMachineFieldValue,
                                        },
                                      }: any) => (
                                        <input
                                          type="checkbox"
                                          className="h-5 w-5 sm:h-4 sm:w-4 mt-0.5 sm:mt-0 text-blue-600 bg-muted border-border rounded focus:ring-blue-500 focus:ring-2 disabled:opacity-50 disabled:cursor-not-allowed touch-target"
                                    ...
                                    <span
                                      className={`text-sm sm:text-base flex-1 break-words ${isPreSelected ? "text-foreground font-medium" : "text-muted-foreground"}`}
                                    >
                                      <span className="block sm:inline">
                                        {machineItem.name}
                                      </span>
                                      <span className="block sm:inline text-xs sm:text-sm text-muted-foreground font-mono">
                                        ({machineItem.machine_id})
                                      </span>
                                      {isPreSelected && (
                                        <span className="block sm:inline ml-0 sm:ml-2 mt-1 sm:mt-0 text-xs text-blue-600">
                                          (Pre-selected)
                                        </span>
                                      )}
                                    </span>
                                  </label>
                                  {values.selected_machine_ids.includes(
                                    machineItem.machine_id,
                                  ) && (
                          ...
                          <div className="text-center py-6">
                            {machineId ? (
                              ...
                              <>
                                <p className="text-sm text-muted-foreground mb-3">
                                  No machine concerns are available for the
                                  selected template and property.
                                </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
                <div>
                  ...
                  <FileUpload
                    onFileSelect={(files) =>
                      handleFileSelection(files, "before", setFieldValue)
                    }
                    maxFiles={1}
                    maxSize={5}
                  ...
                  />
                  {beforeImagePreview && (
                    <div className="mt-3 relative w-full h-40 bg-muted rounded-md overflow-hidden">
                      <PreviewImage
                ...
                <div>
                  ...
                  <FileUpload
                    onFileSelect={(files) =>
                      handleFileSelection(files, "after", setFieldValue)
                    }
                    maxFiles={1}
                    maxSize={5}
              ...
              <div
                className="fixed bottom-[4.5rem] left-0 right-0 z-20 border-t border-border bg-card px-4 py-3 shadow-[0_-4px_16px_rgba(15,23,42,0.08)] md:static md:border-t-0 md:bg-transparent md:px-0 md:py-0 md:shadow-none md:mt-8"
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto">
                    {onCancel && (
                      <button
                        type="button"
                        onClick={onCancel}
                        className="w-full sm:w-auto h-12 sm:h-11 px-6 bg-muted text-muted-foreground font-medium rounded-md shadow-soft hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 transition-colors touch-target"
                        disabled={isSubmitting || isLoading}
                      >
                        Cancel
                      </button>
                  ...
                  <button
                    ...
                    className={`w-full sm:w-auto h-12 sm:h-11 px-6 ${
                      isSubmitting || isLoading
                        ...
                        : "bg-blue-600 hover:bg-blue-700 shadow-soft shadow-blue-600/30"
