...
function arrayBufferToBase64(buffer: ArrayBuffer | null): string {
  ...
  return typeof window !== 'undefined' ? window.btoa(binary).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '') : '';
}
    ...
    try {
      ...
      await fetchWithToken(
        ...
        'POST',
        {
          ...
          keys: {
            p256dh: subscriptionPayload.keys?.p256dh || arrayBufferToBase64(subscription.getKey('p256dh')),
            auth: subscriptionPayload.keys?.auth || arrayBufferToBase64(subscription.getKey('auth')),
          },
        },
      );
      setState('subscribed');
    ...
    <div className={cn('flex flex-col gap-1', className)}>
      ...
      {state === 'denied' && (
        <p className="text-xs font-semibold text-rose-700">
          Browser permission denied. Enable from the site settings to receive push notifications.
        </p>
      )}
      {state === 'unconfigured' && (
