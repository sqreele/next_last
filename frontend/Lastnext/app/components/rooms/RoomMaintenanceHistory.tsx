...
function getTechnician(job: Job): string {
  return getDisplayName(job.user, job.technician_name || job.user_name || 'Unknown technician');
}
...
export function RoomMaintenanceHistory({ jobs, className }: RoomMaintenanceHistoryProps) {
  const [filter, setFilter] = useState<HistoryFilter>('all');

  const stats = useMemo(() => {
    const total = jobs.length;
    const pm = jobs.filter((j) => j.is_preventivemaintenance).length;
    const completed = jobs.filter((j) => normalizeStatus(j.status) === 'completed' || normalizeStatus(j.status) === 'verified').length;
    const open = jobs.filter((j) => isOpen(j)).length;
    const defects = jobs.filter((j) => j.is_defective).length;
    const responseTimes = jobs
      .filter((j) => normalizeStatus(j.status) === 'completed' && j.completed_at)
    ...
    const monthEntries = Array.from(byMonth.entries()).sort(([a], [b]) => a.localeCompare(b));
    const monthMax = Math.max(1, ...monthEntries.map(([, v]) => v));

    return { total, pm, completed, open, defects, avgResponse, lastService, monthEntries, monthMax };
  }, [jobs]);
  ...
  const filtered = useMemo(() => {
    ...
    switch (filter) {
      ...
      case 'completed':
        return base.filter((j) => normalizeStatus(j.status) === 'completed' || normalizeStatus(j.status) === 'verified');
      case 'open':
  ...
  return (
    <section className={cn('space-y-5', className)}>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatTile icon={Activity} label="Total jobs" value={stats.total} tone="primary" />
        <StatTile icon={CheckCircle2} label="Completed" value={stats.completed} tone="success" />
        <StatTile icon={Clock} label="Open" value={stats.open} tone="warning" hint={stats.defects ? `${stats.defects} defects` : undefined} />
        <StatTile icon={Sparkles} label="Preventive" value={stats.pm} tone="info" />
      </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm font-bold text-slate-900">
                {stats.avgResponse !== null ? `${stats.avgResponse} day${stats.avgResponse === 1 ? '' : 's'}` : 'Not enough data'}
              </p>
            </div>
          </div>
        </div>
      ...
      {stats.monthEntries.length > 1 && (
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-900">Jobs over time</h3>
          ...
          <div className="flex h-24 items-end gap-1.5">
            {stats.monthEntries.map(([key, count]) => {
              const height = Math.max(6, Math.round((count / stats.monthMax) * 100));
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="mb-3 flex items-center justify-between gap-2">
          <div>
            <h3 className="text-base font-bold text-slate-900 sm:text-lg">Maintenance history</h3>
              ...
              return (
                <li key={job.job_id} className="relative pl-12">
                  ...
                  <Link
                    ...
                    className="block rounded-xl border border-slate-200 bg-white p-3 transition-colors hover:border-slate-300 hover:bg-slate-50"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="text-sm font-bold text-slate-900 line-clamp-1">{getTitle(job)}</p>
                        <p className="text-xs font-semibold text-slate-500">
                          #{job.job_id} · {new Date(job.created_at).toLocaleDateString('en-US', {
                            month: 'short',
                            day: 'numeric',
                            year: 'numeric',
                          })}
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                      ...
                      <span className="text-slate-500">By {getTechnician(job)}</span>
...
const TONE_STYLES: Record<StatTone, { card: string; icon: string }> = {
  primary: { card: 'border-blue-200 bg-blue-50/40', icon: 'bg-blue-600 text-white' },
  success: { card: 'border-emerald-200 bg-emerald-50/40', icon: 'bg-emerald-600 text-white' },
  warning: { card: 'border-amber-200 bg-amber-50/40', icon: 'bg-amber-500 text-white' },
  info: { card: 'border-indigo-200 bg-indigo-50/40', icon: 'bg-indigo-600 text-white' },
};
...
function StatTile({
  ...
  return (
    <div
      className={cn(
        'flex flex-col gap-1 rounded-2xl border p-3 shadow-sm sm:p-4',
        styles.card,
      )}
    >
      <span className={cn('grid h-9 w-9 place-items-center rounded-xl', styles.icon)}>
        <Icon className="h-4 w-4" />
      </span>
      <p className="text-[11px] font-bold uppercase tracking-wider text-slate-600">{label}</p>
      <p className="text-2xl font-black leading-none text-slate-900 sm:text-3xl">{value}</p>
      {hint ? <p className="text-xs font-semibold text-slate-600">{hint}</p> : null}
    </div>
  );
}
