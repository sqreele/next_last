...
export default function JobsByTopicClient({
  initialJobs,
  topics,
  properties,
}: Props) {
  ...
  const allCount = React.useMemo(
    () => (Array.isArray(propertyScopedJobs) ? propertyScopedJobs.length : 0),
    [propertyScopedJobs],
  );
  ...
  const filtered = React.useMemo(() => {
    ...
    return propertyScopedJobs.filter(
      (job) =>
        Array.isArray(job.topics) &&
        job.topics.some((t) => t.id === selectedTopicId),
    );
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground">
                {selectedTopic
                  ? `${selectedTopic.title} • ${topicCounts.get(selectedTopic.id) || 0} jobs`
                  : `${allCount} total jobs`}
              </p>
            </div>
                ...
                <PopoverTrigger asChild>
                  <Button variant="outline" size="sm" className="h-9 pr-2 pl-2">
                    ...
                    <span className="ml-2 text-xs text-muted-foreground">
                      {selectedTopic
                        ? topicCounts.get(selectedTopic.id) || 0
                        : allCount}
                    </span>
