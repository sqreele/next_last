...
export default function EditPreventiveMaintenancePage() {
      ...
      if (pmId) {
        try {
          ...
          if (data) {
            ...
            console.error(
              "[EDIT FORM] No maintenance data returned for PM ID:",
              pmId,
            );
            setErrors({
          ...
          setErrors({
            general: err.message || "Failed to load maintenance data",
          });
        }
      }
    };
  ...
  const populateForm = (data: PreventiveMaintenance) => {
    ...
    const fixedBeforeImageUrl = data.before_image_url
      ? fixImageUrl(data.before_image_url)
      : null;
    const fixedAfterImageUrl = data.after_image_url
      ? fixImageUrl(data.after_image_url)
      : null;
    ...
    setFormState({
      ...
      topic_ids: data.topics
        ? data.topics.map((t) => (typeof t === "object" ? t.id : t))
        : [],
      procedure_template:
        data.procedure_template || data.procedure_template_id || "",
  ...
  const handleFileUpload = (
    field: "before_image_file" | "after_image_file",
    file: File | null,
  ) => {
    if (file) {
      ...
      if (file.size > MAX_SIZE) {
        setErrors((prev) => ({
          ...prev,
          general: "Image file size must be less than 5MB",
        }));
  ...
  const validateForm = (): boolean => {
    ...
    if (
      ...
      (formState.custom_days === null || formState.custom_days <= 0)
    ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const completionValue =
        formState.completed_date ||
        (completeRequested ? formatDateTimeLocal(new Date()) : "");
      ...
      const updateData: UpdatePreventiveMaintenanceData = {
        ...
        procedure_template:
          typeof formState.procedure_template === "number"
            ? formState.procedure_template
            : formState.procedure_template !== ""
              ? Number(formState.procedure_template)
              : undefined,
      ...
      setErrors({
        general: "Failed to update maintenance record. Please try again.",
      });
    } finally {
  ...
  return (
    <div className="w-full max-w-none px-3 py-4 sm:px-6 sm:py-6 lg:mx-auto lg:max-w-7xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        <div className="bg-card rounded-lg shadow-soft p-3 sm:p-4 md:p-6">
          {/* Basic Information */}
                ...
                >
                  ...
                  {availableMaintenanceTasks.map((task) => (
                    <option key={task.id} value={task.id}>
                      ...
                      {task.frequency?.toUpperCase() || "N/A"}] -{" "}
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          ...
                          handleInputChange(
                            "topic_ids",
                            formState.topic_ids.filter((id) => id !== topic.id),
                          );
                        }
                      }}
                ...
                {formState.before_image_preview ? (
                  <div className="relative">
                    <PreviewImage
                      ...
                      onError={() => {
                        console.error(
                          "[EDIT FORM] Before image failed to load",
                        );
                        console.error(
                          "[EDIT FORM] Failed URL:",
                          formState.before_image_preview,
                        );
                      }}
                    />
                  ...
                  <div className="border-2 border-dashed border-border rounded-lg p-4 sm:p-6 text-center">
                    ...
                    <div className="mt-2">
                      <label className="cursor-pointer touch-feedback">
                        <span className="text-blue-600 hover:text-blue-700 text-sm sm:text-base inline-block py-2 px-4 rounded-md hover:bg-blue-50 touch-target min-h-[44px] flex items-center justify-center">
                          Upload image
                        </span>
                        <input
                          type="file"
          ...
          {/* Action Buttons */}
          <div className="flex flex-col-reverse sm:flex-row sm:justify-between gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-border">
            <button
              type="button"
              onClick={() =>
