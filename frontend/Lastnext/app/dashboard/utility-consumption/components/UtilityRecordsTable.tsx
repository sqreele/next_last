...
export default function UtilityRecordsTable({ rows, allRows = rows }: { rows: UtilityConsumptionRow[]; allRows?: UtilityConsumptionRow[] }) {
  ...
  const cells = (row: UtilityConsumptionRow) => [
    [t("utility.totalKwh"), `${number.format(row.totalkwh)} kWh`],
    [t("utility.onPeak"), `${number.format(row.onpeakkwh)} kWh`],
    [t("utility.offPeak"), `${number.format(row.offpeakkwh)} kWh`],
    [t("utility.water"), `${number.format(row.water)} m³`],
    [t("utility.electricityCost"), `${number.format(row.totalelectricity)} THB`],
    [t("utility.budget"), `${number.format(row.electricity_cost_budget)} THB`],
    [t("utility.previousPeriod"), change(row.totalelectricity, previousCost(row))],
  ];
  ...
  return <section aria-labelledby="utility-records-heading" className="rounded-xl border border-border bg-card shadow-soft">
    ...
    <div className="hidden max-h-[32rem] overflow-auto md:block"><table className="w-full min-w-[900px] text-sm"><thead className="sticky top-0 z-10 bg-muted"><tr><th className="p-3 text-left">{t("utility.month")}</th><th className="p-3 text-right">{t("utility.totalKwh")}</th><th className="p-3 text-right">{t("utility.onPeak")}</th><th className="p-3 text-right">{t("utility.offPeak")}</th><th className="p-3 text-right">{t("utility.water")}</th><th className="p-3 text-right">{t("utility.electricityCost")}</th><th className="p-3 text-right">{t("utility.budget")}</th><th className="p-3 text-right">{t("utility.previousPeriod")}</th></tr></thead><tbody>{ordered.map((row) => <tr className="border-t border-border" key={`${row.year}-${row.month}`}><td className="p-3 font-medium">{row.month} {row.year}</td>{cells(row).map(([label, value]) => <td className="p-3 text-right tabular-nums" key={label}>{value}</td>)}</tr>)}</tbody></table></div>
  </section>;
}
