...
class PreventiveMaintenanceService {
      ...
      if (Array.isArray(item.machines)) {
        const arrayMatch = item.machines.some((machine) => {
          ...
          if (typeof machine === "object" && machine !== null) {
            return (
              (machine as any).machine_id === machineId ||
  ...
  async getAllPreventiveMaintenance(
    ...
    try {
      ...
      cleanParams.page_size = params?.page_size
        ? parseInt(String(params.page_size), 10)
        : 10;
  ...
  async getPreventiveMaintenanceByMachine(
    ...
    if (!machineId) {
      ...
      return {
        success: false,
        message: "Machine ID is required to fetch related maintenance",
      };
    }
    ...
    try {
      ...
      return {
        success: false,
        message: `No maintenance items found for machine ${machineId}`,
      };
    } catch (error: any) {
      console.error(
        `Service error fetching maintenance for machine ${machineId}:`,
        error,
      );
      throw handleApiError(error);
    }
  ...
  async getPMMasterPlans(
    ...
    try {
      ...
      return {
        success: true,
        data: response.data,
        message: "PM master plans fetched successfully",
      };
    } catch (error: any) {
      console.error("Service error fetching PM master plans:", error);
      throw handleApiError(error);
    }
  }
  ...
  async createPMMasterPlan(
    ...
    try {
      ...
      return {
        success: true,
        data: response.data,
        message: "PM master plan created successfully",
      };
    } catch (error: any) {
      console.error("Service error creating PM master plan:", error);
      throw handleApiError(error);
    }
  }
  ...
  async materializePMMasterPlans(
    ...
    try {
      ...
      return {
        success: true,
        data: response.data,
        message: "PM master plans materialized successfully",
      };
    } catch (error: any) {
      console.error("Service error materializing PM master plans:", error);
      throw handleApiError(error);
    }
  }
  ...
  async createPreventiveMaintenance(
    ...
    try {
      ...
      if (
        data.machine_ids &&
        Array.isArray(data.machine_ids) &&
        data.machine_ids.length > 0
      ) {
        ...
        throw new Error(
          "At least one machine is required. machine_ids must be an array.",
        );
      }
  ...
  async updatePreventiveMaintenance(
    ...
    try {
      ...
      if (data.pmtitle !== undefined)
        formData.append(
          "pmtitle",
          data.pmtitle.trim() || "Untitled Maintenance",
        );
      ...
      if (data.custom_days !== undefined) {
        formData.append(
          "custom_days",
          data.custom_days != null ? String(data.custom_days) : "",
        );
      }
      if (data.notes !== undefined)
        formData.append("notes", data.notes?.trim() || "");
  ...
  async completePreventiveMaintenance(
    ...
    if (!id) {
      ...
      return {
        success: false,
        message: "PM ID is required to mark as complete",
      };
    }
    ...
    try {
      ...
      if (upcomingResponse.success && upcomingResponse.data) {
        ...
        return {
          success: true,
          data: enhancedStats,
          message: "Enhanced statistics fetched successfully",
        };
      }
    ...
    try {
      ...
      if (error.status === 403) {
        return {
          ...
          message:
            ...
            "You don't have permission to delete this maintenance record. Please contact an administrator.",
        };
      }
      ...
      if (error.status === 404) {
        return {
          success: false,
          message:
            "This maintenance record no longer exists or has already been deleted.",
        };
      }
