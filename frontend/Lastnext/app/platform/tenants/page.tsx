...
export default async function PlatformTenantsPage() {
  ...
  return <PlatformPage title="Tenants"><div className="overflow-x-auto rounded-lg border"><table className="w-full text-sm"><thead><tr className="border-b bg-muted/50"><th className="p-3 text-left">Tenant</th><th className="p-3 text-left">Plan</th><th className="p-3 text-left">Status</th><th className="p-3 text-left">Properties</th><th className="p-3 text-left">Users</th></tr></thead><tbody>{rows.map((row) => { const subscription = row.subscription as Record<string, unknown>; const plan = subscription?.plan as Record<string, unknown>; return <tr key={String(row.tenant_id)} className="border-b"><td className="p-3"><Link className="font-medium underline" href={`/platform/tenants/${row.tenant_id}`}>{String(row.name)}</Link><div className="text-xs text-muted-foreground">{String(row.tenant_id)}</div></td><td className="p-3">{String(plan?.name ?? "—")}</td><td className="p-3">{String(subscription?.entitlement_level ?? row.status)}</td><td className="p-3">{String(row.property_count)}</td><td className="p-3">{String(row.active_membership_count)}</td></tr>; })}</tbody></table></div></PlatformPage>;
}
