...
/* Edge middleware cannot connect to the Docker-only Redis TCP endpoint. It
 * rejects all legacy cookie formats here; Node BFF routes load the canonical
 * server session and fail closed before forwarding authenticated requests. */
...
export async function middleware(request: NextRequest) {
  ...
  // Only an opaque v2 reference is accepted. Bearer and refresh tokens remain
  // inside the encrypted Redis value and are never placed in this cookie.
