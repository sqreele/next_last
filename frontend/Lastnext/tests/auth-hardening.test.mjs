...
test('login and callback retain state, require nonce and PKCE, and fail closed', async () => {
  ...
  const login = await readFile(new URL('app/lib/auth0/login-flow.ts', root), 'utf8');
  ...
  assert.match(canonicalLogin, /beginHardenedAuth0Login\(request\)/);
  ...
  assert.match(login, /auth0_pkce_verifier/);
  assert.match(login, /createAuth0AuthorizationTransaction/);
  assert.doesNotMatch(rootLogin, /\/authorize/);
  assert.doesNotMatch(canonicalLogin, /\/authorize/);
  assert.doesNotMatch(catchAllLogin, /\/authorize/);
  assert.match(callback, /validateOAuthCallbackTransaction/);
